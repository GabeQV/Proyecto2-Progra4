...
import com.example.proyecto1.dto.AuthResponse;
import com.example.proyecto1.dto.LoginRequest;
import com.example.proyecto1.dto.RegistroEmpresaRequest;
import com.example.proyecto1.dto.RegistroOferenteRequest;
import com.example.proyecto1.logic.Service;
import com.example.proyecto1.security.CustomUserDetailsService;
import com.example.proyecto1.security.JwtTokenProvider;
...
public class AuthController {
    ...
    public ResponseEntity<?> refresh(@RequestBody Map<String, String> body) {
        ...
        try {
            ...
            String nuevoAccessToken = jwtTokenProvider.generateToken(userDetails);
    ...
    public ResponseEntity<?> registrarEmpresa(
        ...
        try {
            service.registrarEmpresa(
                    ...
                    request.getClave(),
                    request.getNombre(),
                    request.getLocalizacion(),
                    request.getTelefono(),
                    request.getDescripcion()
            );
            return ResponseEntity.ok(
                    Map.of("mensaje", "Empresa registrada. Pendiente de aprobación por el administrador.")
            );
    ...
    public ResponseEntity<?> registrarOferente(
            @Valid @RequestBody RegistroOferenteRequest request) {
        try {
            ...
            return ResponseEntity.ok(
                    Map.of("mensaje", "Oferente registrado. Pendiente de aprobación por el administrador.")
            );
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(Map.of("error", e.getMessage()));
        }
