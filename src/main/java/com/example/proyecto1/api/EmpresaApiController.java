...
import com.example.proyecto1.dto.CrearPuestoRequest;
...
public class EmpresaApiController {
    ...
    @GetMapping("/puestos")
    public ResponseEntity<?> getMisPuestos(
            @AuthenticationPrincipal UserDetails userDetails) {
        List<Puesto> puestos = service.getPuestosDeEmpresa(userDetails.getUsername());
    ...
    public ResponseEntity<?> crearPuesto(
            @Valid @RequestBody CrearPuestoRequest request,
            @AuthenticationPrincipal UserDetails userDetails) {
        try {
    ...
    public ResponseEntity<?> buscarCandidatos(@PathVariable Integer id) {
        try {
            ...
            return ResponseEntity.internalServerError()
                    .body(Map.of("error", "Error al buscar candidatos"));
        }
    }
